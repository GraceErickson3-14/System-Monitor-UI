...
class programMain
{
    ...
    static void Main(string[] args)
    {
        ...
        foreach (var line in lines)
        {
            ...
            if (line.StartsWith("windows_cs_physical_memory_bytes"))
            {
                var newCPUCstateSeconds = new windowsCsPhysicalMemorybyte();
                newCPUCstateSeconds.Value = Regex.Match(line, @"\d+(\.\d+)?([eE][+-]?\d+)").Value;
                Console.WriteLine(newCPUCstateSeconds.Value);
            }

            // windows_os_virtual_memory_bytes OperatingSystem.TotalVirtualMemorySize
            if (line.StartsWith("windows_os_virtual_memory_bytes"))
            {
                var newVirtual = new windowsOSVirtual();
                newVirtual.value = Regex.Match(line, @"\d+(\.\d+)?([eE][+-]?\d+)").Value;
                Console.WriteLine(newVirtual.value);
            }
            //windows_cpu_cstate_seconds_total Time spent in low - power idle state
            if (line.StartsWith("windows_cpu_cstate_seconds_total"))
            {
                ...
                cpuCstate.value = double.Parse(Regex.Match(line, @"\d+(?:\.\d+)?(?=\s*$)").ToString());
            }
            if (line.StartsWith("windows_logical_disk_size_bytes"))
            {
            ...
            if (line.StartsWith("windows_logical_disk_free_bytes"))
            {
                var windowDiskBytes = new windowsSizeBytes();
                windowDiskBytes.Names = "windows_logical_disk_size_bytes";
                windowDiskBytes.Values = Regex.Match(line, @"\d+\.\d+([eE][-+]?\d+)?").ToString();
